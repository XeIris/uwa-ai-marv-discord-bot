...
- **Docker** (`Dockerfile`): multi-stage — `oven/bun:1.4` builder installs deps and fetches the GM
  soundfont (checksum-verified, layer-cached), then `oven/bun:1.4-slim` runtime as non-root user
...
- **The Bun version is pinned in three places that must move together:** `.bun-version`,
  `engines.bun` in `package.json`, and the two `oven/bun:` tags above.
